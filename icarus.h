...
*/
tensor *tensor_apply_unop(tensor *t, f32 (*op)(f32, f32), f32 arg) {
...
tensor *tensor_relu(tensor *t) { return tensor_apply_unop(t, &_relu, -1); }
tensor *tensor_softmax(tensor *t) {
	tensor *max = tensor_max(t, 1, true);
	tensor *top = tensor_exp(tensor_sub(t, max));
	tensor *bottom = tensor_sum(top, 1, true);
	return tensor_div(top, bottom);
}
...
* Reduce Ops
*/
tensor *tensor_apply_reduceop(tensor *t, int axis, bool keepdims, void (*op)(f32*, f32), f32 init) {
	int *nshape;
	if (! keepdims) {
		nshape = (int*) malloc((t->ndims-1) * sizeof(int));
		for (int i=0; i<axis; ++i) nshape[i]=t->shape[i];
		for (int j=axis; j<t->ndims-1; ++j) nshape[j]=t->shape[j+1];
	} else {
		nshape = (int*) malloc(t->ndims * sizeof(int));
		for (int i=0; i<t->ndims; ++i) nshape[i] = i == axis ? 1 : t->shape[i];
	}
	...
	do {
		if (!keepdims) {
			...
			memcpy(iter, riter, t->ndims * sizeof(i32));
		}
...
tensor *tensor_sum(tensor *t, int axis, bool keepdims) { return tensor_apply_reduceop(t, axis, keepdims, &__sum, 0); }
tensor *tensor_max(tensor *t, int axis, bool keepdims) { return tensor_apply_reduceop(t, axis, keepdims, &__max, -FP_INFINITE); }
tensor *tensor_mean(tensor *t, int axis, bool keepdims) { return tensor_div_scalar(tensor_sum(t, axis, keepdims), t->shape[axis]); }
